        ...
        where C : Monoid<C>
    {
        ...
        (C, Seq<Option<O>>) ctr(SpanArray<(C, Seq<Option<O>>)> v)
        {
            ...
            else if (quot == 0)
            {
                ...
                var (pc, po) = get(v, 0, y);
                return (p.cost(o).Combine(pc), Optional(o).Cons(po));
            }
            else if (rem == 0)
            {
                var x = quot - 1;
            ...
            else
            {
                ...
                else
                {
                    ...
                    var item1 = (p.cost(c1).Combine(top.Item1), Optional(c1).Cons(top.Item2));

                    var c2    = p.insert(position(left.Item2), d);
                    var item2 = (p.cost(c2).Combine(left.Item1), Optional(c2).Cons(left.Item2));

                    var c3    = p.substitute(position(tl.Item2), s, d);
                    var item3 = (p.cost(c3).Combine(tl.Item1), Optional(c3).Cons(tl.Item2));
